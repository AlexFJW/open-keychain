...
public class ProviderWriter {
    ...
    // be wary of this later
    protected boolean mConsolidatingOwnPublic;
                ...
                for (CanonicalizedPublicKey key : keyRing.publicKeyIterator()) {
                    ...
                    if (masterKeyId == keyId) {
                        if (key.getKeyUsage() == null) {
                            ...
                            mProviderHelper.log(ProviderHelper.LOG_TYPES_FLAG_MASTER[(c ? 1 : 0) + (e ? 2 : 0) + (s ? 4 : 0) + (a ? 8 : 0)]);
                        }
                    } else {
                        if (key.getKeyUsage() == null) {
                            mProviderHelper.log(LogType.MSG_IP_SUBKEY_FLAGS_UNSPECIFIED);
                        } else {
                            mProviderHelper.log(ProviderHelper.LOG_TYPES_FLAG_SUBKEY[(c ? 1 : 0) + (e ? 2 : 0) + (s ? 4 : 0) + (a ? 8 : 0)]);
                        }
                    }
        ...
        try {
            ...
            if (!hasSecretKeyRing) {
                ...
                if (secretRing != null) {
                    ...
                    secretKeyData = mProviderHelper.getSecretKeyringData(masterKeyId);
